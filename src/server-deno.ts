...
import { serve, serveTls } from "https://deno.land/std@0.174.0/http/server.ts";
...
function systemUp() {
  ...
  // deno.land/manual@v1.30.0/runtime/http_server_apis
  async function startDoh() {
    if (terminateTls()) {
      // deno.land/std@0.174.0/http/server.ts?s=serveTls
      serveTls(serveDoh, {
        ...dohConnOpts,
        ...tlsOpts,
        ...httpOpts,
      ...
      // deno.land/std@0.174.0/http/server.ts?s=serve
